...
public class menu_admin extends AppCompatActivity {
    ...
    public boolean onOptionsItemSelected(MenuItem item){
        switch (item.getItemId()){
            case R.id.logoutBtn :
                AlertDialog.Builder builder = new AlertDialog.Builder(menu_admin.this);
                builder.setMessage("Apakah Anda Ingin Logout?").setNegativeButton("No", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialogInterface, int which) {
                        Toast.makeText(menu_admin.this, "Cancel Logout", Toast.LENGTH_SHORT).show();
                    }
                })
                        .setPositiveButton("Yes", new DialogInterface.OnClickListener() {
                            @Override
                            public void onClick(DialogInterface dialogInterface, int which) {
                                Toast.makeText(menu_admin.this, "Berhasil LogOut", Toast.LENGTH_SHORT).show();
                                Intent intent = new Intent(menu_admin.this, Login_face.class);
                                startActivity(intent);
                                menu_admin.super.onDestroy();
                            }
                        });
